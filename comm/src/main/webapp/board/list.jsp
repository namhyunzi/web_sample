...
<%
	...
	/*
		요청 URL
			...
			localhost/comm/board/list.jsp?page=xx&opt=xxx&keyword=xxx
		요청 파라미터
			page=xx
			opt=xxx
			keyword=xxx
			...
			* page는 조회했을 때 null이거나 숫자가 조회된다.
				* null일때는 네비바의 메뉴를 클릭했을 때다.
					comm/board/list.jsp
			* opt와 keyword는 조회했을 때 null, 빈문자열, 문자열이 조회된다.
				* null일때는 네비바의 메뉴를 클릭했을 때다.
					comm/board/list.jsp
				* 빈 문자열일 때는 게시글 목록페이지에서 검색없이 단순히 페이지 번호만 클릭했을 때다.
					comm/board/list.jsp?page=2&opt=&keyword=
				* 값이 있을 때는 옵션과 검색어를 입력하고 검색버튼을 클릭했을 때다.
					comm/board/lsit.jsp?page=1&opt=title&keyword=연습
				* 값이 있을 때는 검색된 목록페이지에서 페이지번호를 클릭했을 때다.
					comm/board/lsit.jsp?page=3&opt=title&keyword=연습
	...
	*/
	...
	// 3. COMM_BOARDS 테이블에 대한 CRUD기능이 구현된 BoardDao객체를 생성한다.
...
%>
			<div class="d-flex justify-content-between my-3">
				<select class="form-selct w-25" name="rows" onchange="changeRows()">
					<option value="10" <%=10 == rows ? "selected" : "" %>> 10개씩</option>
					<option value="20" <%=20 == rows ? "selected" : "" %>> 20개씩</option>
					<option value="30" <%=50 == rows ? "selected" : "" %>> 50개씩</option>
				</select>
				<select class="form-selct w-25" name="sort" onchange="changeSort()">
...
<script type="text/javascript">
	...
	// 페이지 번호를 클릭했을 때 실행되는 이벤트 핸들러 함수다.
	...
	//   클릭한 페이지번호도 전달되어야 한다.
	// * 따라서, 페이지번호를 클릭했을 때는 검색폼을 제출해야 한다.
