...
public class LoginCheckFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain filterChain) throws IOException, ServletException {
		...
		System.out.print("요청 URI: " + requestURI);
		
		if (urlSet.contains(requestURI)) {
			System.out.print("로그인이 필요한 요청이다.");
			// 로그인이 필요한 요청이다.
			HttpSession session = httpReq.getSession();
			if (session.getAttribute("LOGIN_USER") != null) {
				System.out.print("로그인된 상태다.");
				filterChain.doFilter(req, res);
			} else {
				System.out.print("로그인이 안된 상태다.");
				httpRes.sendRedirect("/comm/loginform.jsp?error=deny");
			}
		} else {
			// 로그인이 필요하지 않은 요청이다.
			System.out.print("로그인이 필요하지 않은 요청이다.");
			filterChain.doFilter(req, res);
		}
